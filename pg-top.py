...
user_cols_select_query_for_schema = user_cols_select_query + \
    "WHERE U.schemaname = '%s'"
...
class PgTop:
    ...
    def fetch_user_cols(self):
        ...
        for name, con in self.con.items():
            if self.opts.schema:
                data += DB.execute_fetchall(
                    con, user_cols_select_query_for_schema % (cols, self.opts.schema))
            else:
    ...
    def _refresh(self):
        ...
        for c in self.user_cols_meta:
            if c[USER_COL_TYPE] == "int":
                ...
                fmt.append("%%%ds" % c[1])
        fmt_data = " ".join(fmt)
        fmt_header = " ".join(["%%%ds" % c[USER_COL_WIDTH]
                               for c in self.user_cols_meta])
    ...
    def shift_sorted_col(self, shift):
        self.user_cols_sorted = (len(
            self.user_cols_meta) + self.user_cols_sorted + shift) % len(self.user_cols_meta)
...
def main():
    ...
    p.add_option("-d", "--delay",   type=int, default=2,
                 help="delay between database poll (sec)")
    p.add_option("-n", "--count",   type=int, default=0,
                 help="exit after COUNT iterations")
    p.add_option("-a", "--abs",     action="store_true",
                 help="show absolute values, not rates")
    ...
    if HAS_PA:
        ...
        g.add_option("-c", "--config",  type="string", default=None,
                     help="PA config file [default: %default]")
        g.add_option("", "--poa",       action="store_true",
                     help="connect to POA only")
        g.add_option("", "--pba",       action="store_true",
                     help="connect to PBA only")
        p.add_option_group(g)
        defdb = "plesk"
        defusr = "plesk"

    g = OptionGroup(
        p, "If you don't have pa.conf" if HAS_PA else "Database credentials")
    g.add_option("", "--db-host",   type="string", help="database hostname/IP")
    g.add_option("", "--db-port",   type="string",
                 default=5432, help="database port")
    g.add_option("", "--db-name",   type="string", default=defdb,
                 help="database name [default: %default]")
    g.add_option("", "--db-user",   type="string", default=defusr,
                 help="database username [default: %default]")
    g.add_option("", "--db-pass",   type="string",
    ...
    logging.basicConfig(
        level=loglevel, format="%(asctime)s - %(module)s - %(levelname)s - %(message)s")
    ...
    if HAS_PA and opts.config:
        ...
        if (non or opts.pba) and cfg.pba_db.host:
            dbs.append(DB(cfg.pba_db.ip, cfg.pba_db.db_port,
                          cfg.pba_db.db_name, cfg.pba_db.db_user, cfg.pba_db.db_pass))
        if (non or opts.poa) and cfg.poa_db.host:
            dbs.append(DB(cfg.poa_db.ip, cfg.poa_db.db_port,
                          cfg.poa_db.db_name, cfg.poa_db.db_user, cfg.poa_db.db_pass))
    else:
        dbs.append(DB(opts.db_host, opts.db_port,
                      opts.db_name, opts.db_user, opts.db_pass))
