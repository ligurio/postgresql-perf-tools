...
class PgVersion:
    ...
    def __init__(self, con):
        ...
        self.maj = DB.execute_fetchval(
            con, "SELECT substring(version() from $$(\d+)\.\d+\.\d+$$)::smallint;")
        self.min = DB.execute_fetchval(
            con, "SELECT substring(version() from $$\d+\.(\d+)\.\d+$$)::smallint;")
...
class PgStatStore:
    ...
    def update(self):
        ...
        ret = DB.execute_fetchone(con, "SELECT %s FROM %s" %
                                  (", ".join(query), self.table))
        self.store = {}
        n = 0
        for c in self.cols:
            self.store[c] = ret[n]
...
class pgsDbSize(DbStatCounter):
    ...
    def update_action(self, **kwargs):
        self.val = DB.execute_fetchval(
            con, "select pg_database_size('%s')" % opts.db_name)
        self.val /= 1024
...
class pgsLockWait(DbStatCounter):
    ...
    def update_action(self):
        self.val = DB.execute_fetchval(
            con, "SELECT COUNT(*) FROM pg_locks WHERE NOT granted")
...
class pgsProcsIdletxn(DbStatCounter):
    ...
    def update_action(self):
        ...
        self.val = int(self.store.store[
                       key]) if self.store.store.has_key(key) else 0
...
class PgStats:
    ...
    def __init__(self):
        ...
        s_db = PgStatStore("pg_stat_database", [
                           "xact_commit", "xact_rollback", "blks_read", "blks_hit"])
        s_ut = PgStatStore("pg_stat_user_tables", [
                           "n_tup_ins", "n_tup_upd", "n_tup_del"])
        s_utb = PgStatStoreBigUserTables(
            ["idx_scan", "seq_scan", "seq_tup_read"])
        ...
        self.groups = [
            ...
            ("Locks", [pgsLockWait()] +
             ([pgsDeadlocks(s_db)] if pg_ver.ge(9, 2) else [])),
        ...
        if pg_ver.ge(9, 2):
            self.groups.append(
                ("Disk Wait", [pgsIoReadWa(s_db), pgsIoWriteWa(s_db)]))
    ...
    def init(self):
        ...
        for group in self.groups:
            ...
            self.hdr_titles += " " + \
                group[0].rjust(
                    sum(c.width + 1 for c in group[1]) - 1) + self.sep
    ...
    def header(self):
        ...
        if opts.abs:
            ...
            metrics = [c.metric if c.absolute else "%s/s" %
                       c.metric for c in self.counters]
    ...
    def print_row(self):
        if opts.abs:
            ...
            for c in self.counters:
                ...
                fmt = c.rate_fmt if c.rate_fmt else (
                    "%.1f" if r < 100 else "%.0f")
                vals.append(fmt % r)
...
def main():
    ...
    if HAS_PA:
        ...
        g.add_option("", "--pba",       action="store_true",
                     help="connect to PBA (POA is default)")
    ...
    db = DB(opts.db_host, opts.db_port,
